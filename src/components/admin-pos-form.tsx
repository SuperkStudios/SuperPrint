      ...
      <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="grid gap-4 rounded-md border bg-card p-4">
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <Field label="Email"><Input type="email" value={customerEmail} onChange={(event) => setCustomerEmail(event.target.value)} required /></Field>
          </div>
            ...
            {lines.map((line, index) => {
              ...
              return (
                <div key={index} className="grid gap-3 rounded-md border bg-background p-3">
                  <div className="grid gap-3 md:grid-cols-[1fr_96px_120px_auto]">
                    <Field label="Product">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <Button key={String(method)} type="button" variant={paymentMethod === method ? "default" : "outline"} onClick={() => setPaymentMethod(String(method))}>
                ...
                {method === "STRIPE_TERMINAL" ? "Terminal" : method === "STRIPE_MANUAL" ? "Manual" : "Cash"}
              </Button>
            ))}
          </div>
