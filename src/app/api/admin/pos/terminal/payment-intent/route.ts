...
import { createTerminalOrderPayment } from "@/services/in-person-orders";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createTerminalOrderPayment({
      ...body,
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not start Terminal payment." }, { status: 400 });
  }
}
